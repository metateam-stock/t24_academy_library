...
public interface RentalManageRepository extends JpaRepository<RentalManage, Long> {
    ...
    @Query("SELECT COUNT(*) " +
            ...
            "WHERE rm.status in (0,1)  "+
            "AND (rm.expectedRentalOn <= ?1 AND rm.expectedReturnOn >= ?1) " +
            "AND rm.stock.id in ?2")
    Long findByDailySchedule(Date currentDate,List<String> stock_id);
