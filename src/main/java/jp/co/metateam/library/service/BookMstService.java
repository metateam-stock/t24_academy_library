...
public class BookMstService {
    ...
    public List<BookMstDto> findTitle() {
        List<BookMst> bookMstAvailable = this.bookMstRepository.findTitle();
        List<BookMstDto> stockCalendar = new ArrayList<BookMstDto>();

        for (int i = 0; i < bookMstAvailable.size(); i++) {
            BookMst book = bookMstAvailable.get(i);
            // BookMstDtoの変数宣言し、新しいものを作る。
            BookMstDto bookMstDto = new BookMstDto();
            // タイトル取得
            bookMstDto.setTitle(book.getTitle());
            bookMstDto.setId(book.getId());
            // 取得したタイトルをストックカレンダーに追加する
            stockCalendar.add(bookMstDto);
        }
